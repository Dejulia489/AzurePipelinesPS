function Update-APPullRequestThread
{
    <#
    .SYNOPSIS
    ...
    .PARAMETER CommentThreadStatus

    The status of the thread. Valid values are: active, byDesign, closed, fixed, pending, unknown, wontFix.
    ...
    Update-APPullRequestThread @splat
    ...
    {
        $body = @{}
        If($CommentThreadStatus){
            $body.status = $CommentThreadStatus
        }
        If($Comment){
            $body.comments = @(
                @{
        ...
        $apiEndpoint = (Get-APApiEndpoint -ApiType 'git-pullRequests-threadId') -f $RepositoryId, $PullRequestId, $ThreadId
        $setAPUriSplat = @{
